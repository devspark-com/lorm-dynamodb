...
public class DynamoDBBaseRepositoryTest extends BaseIntegrationTest {
	...
	Repository<Merchant> merchantRepository = entityManager.getRepository(Merchant.class);
	...
	// remove all expenses
	Repository<Expense> expenseRepository = entityManager.getRepository(Expense.class);
	...
	Repository<SampleEntity> repository = entityManager.getRepository(SampleEntity.class);
	...
	Assert.assertEquals("some embedded field", savedSampleEntity.getEmbedded().getSomeField());
