...
public class DynamoDBEntityManager extends AbstractEntityManagerImpl {
    ...
    public DynamoDBEntityManager(String host, String port, String username, String password) {
	...
	if (properties.get(ACCESS_KEY) == null || properties.get(SECRET_KEY) == null) {
	    ...
	    dynamoDB = new AmazonDynamoDBClient(new BasicAWSCredentials(properties.get(ACCESS_KEY),
	...
	if (properties.get(HOST) != null) {
	    ...
	    dynamoDB.setEndpoint("http://" + properties.get(HOST) + ":" + properties.get(PORT));
	}
    }
	...
	getAllRepositories().put(entityClass, new DynamoDBBaseRepository<T>(new DynamoDB(dynamoDB),
		entityToItemMapper, itemToEntityMapper, entitySchemaSupport, entityClass));
    }
